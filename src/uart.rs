...
impl<'a> UartRx<'a, Async> {
    ...
    async fn read_buffered(&mut self, buf: &mut [u8]) -> Result<()> {
        ...
        while bytes_read < buf.len() {
            if rx_dma.is_active() {
                let remaining_bytes = rx_dma.get_xfer_count() as usize;
            ...
            if buffer_config.write_index != buffer_config.read_index {
                // handle roll over
                if buffer_config.write_index > buffer_config.read_index {
                    let incoming_slice = &buffer_config.buffer[buffer_config.read_index..buffer_config.write_index];
                    let start = bytes_read;
                    let stop = (bytes_read + incoming_slice.len()).min(buf.len());
                    let read_len = stop - start;
                    let outgoing_slice = &mut buf[start..stop];
                    ...
                    let slice_first_part = &buffer_config.buffer[buffer_config.read_index..];

                    let start = bytes_read;
                    let stop = (bytes_read + slice_first_part.len()).min(buf.len());
                    let read_len = stop - start;
                    let outgoing_slice = &mut buf[start..stop];
                    outgoing_slice.copy_from_slice(slice_first_part);
                    ...
                    if bytes_read < buf.len() && buffer_config.write_index > 0 {
                        ...
                        let slice_second_part = &buffer_config.buffer[0..buffer_config.write_index];

                        let start = bytes_read;
                        let stop = (bytes_read + slice_second_part.len()).min(buf.len());
                        let read_len = stop - start;
                        let outgoing_slice = &mut buf[start..stop];
                        outgoing_slice.copy_from_slice(slice_second_part);
